  ...
  return (
    <>
      <section className="relative min-h-[75vh] flex items-end">
        <Image className="w-full h-full object-cover" fill src={BannerRes} alt="AboutUs Banner" />
      ...
      <section className="bg-dark-purple text-white py-8">
        <div className="container mx-auto">
          ...
          <div className="relative rounded-2xl aspect-square overflow-hidden mb-6">
            <div className="bg-purple-gradient w-full h-full relative z-10"></div>
            <Image className="w-full h-full object-cover" fill src={FoundersImg} alt="Founders" />
            <div className="absolute bottom-0 w-full grid grid-cols-3 justify-items-center gap-1 p-4 text-small font-bold text-light-yellow z-10">
              <span>Benoit Raby</span>
              <span>Luc Lemaire</span>
              <span>Javier Galvis</span>
            </div>
          </div>
          <CustomButton block>Conoce sus biografías</CustomButton>
        </div>
      </section>
      ...
      <section className="relative pt-20 py-8">
        ...
        <div className="container mx-auto">
          <h2 className="text-title text-center mb-10">
            <span className="text-subtitle">Los 6 principios de</span> <br /> Nuestras <br /> creaciones
          </h2>
            ...
            <div className="mx-auto w-full rounded-2xl p-2">
              {tabItems.map((item, index) => (
                <Disclosure key={`tab-item-${item.title}`} defaultOpen={index == 0 ? true : false}>
                  {({ open }) => (
                    <>
                      <Disclosure.Button className={`${open ? 'bg-violet text-white px-6' : ''} w-full flex items-center rounded-full gap-6 py-3 transition-colors`}>
                        <div className={`h-[60px] w-[60px] grid place-content-center rounded-full p-3x border ${open ? 'border-transparent' : 'border-purple'}`}>
                          <ArrowDownIcon className={`${!open && '-rotate-90'} transition`} />
                        </div>
                      ...
                      >
                        <Disclosure.Panel className="pt-6 mb-8" static>
                          ...
                          <Image className="w-full rounded-2xl aspect-square object-cover" src={TabAImg} alt="Tab A" />
                        </Disclosure.Panel>
                      </Transition>
                    </>
                  )}
